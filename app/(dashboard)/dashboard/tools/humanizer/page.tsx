...
export default async function HumanizerPage() {
  const isFavorite = await getIsFavorite("humanizer");

  return (
    <div className="space-y-6">
      <div>
        <p className="font-mono text-[11px] uppercase tracking-wider text-paper-400">AI tools</p>
        <h1 className="font-display text-xl font-medium text-paper-900">Humanizer</h1>
      </div>
      <SimpleToolForm toolSlug="humanizer" isFavorite={isFavorite} />
    </div>
  );
}